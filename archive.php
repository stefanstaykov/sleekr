    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">
                <?php if ( is_category() ) {
                    _e( 'Category Archive ', 'sleekr' );
                    single_cat_title(); 
                } else if ( is_author() ) { 
                    $curauth = (isset($_GET['author_name'])) ? get_user_by('slug', $author_name) : get_userdata(intval($author));
                    _e( 'Author Archive ', 'sleekr' );
                    echo $curauth->display_name;
                } else if ( is_day() ) {
                    _e( 'Daily Archive ', 'sleekr' );
                    echo (' '. get_the_time('F') .' '. get_the_time('jS') .' ');
                } else if ( is_month() ) {
                    _e( 'Monthly Archive ', 'sleekr' );
                    echo get_the_time('F');
                } else if ( is_year() ) {
                    _e( 'Yearly Archive ', 'sleekr' );
                    echo get_the_time('Y');
                } else if ( is_tag() ) {
                    _e( 'Tag Archive ', 'sleekr' );
                    single_tag_title();
                }?> 
            </h1>
...
<div class="row" style="margin-left:0">
  <div class="col-md-8 well">
	...
	<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
        ...
        <?php if ( !is_author() ) { echo '<p class="display-author">'; _e('by ','sleekr'); the_author_posts_link(); echo'</p>'; } ?>
